...
function buildAIDiscoveryPrompt(mode, ctx) {
    ...
    if (mode === 'contacts') {
        ...
        return [
            ...
            '',
            'PRAVIDLA PRO JMÉNO KONTAKTNÍ OSOBY (KRITICKÉ — nejčastější zdroj halucinací):',
            '- NIKDY si jméno osoby nevymýšlej a neodhaduj. Uveď konkrétní osobu POUZE pokud jsi ji našel v konkrétním ověřitelném zdroji (justice.cz / obchodní rejstřík, ARES, firemní web sekce „o nás / management / tým", oficiální LinkedIn firmy).',
            '- Pokud osobu nemůžeš ověřit, vrať "kontaktniOsoba": null a "role": null — kontakt zůstane na úrovni firmy. RADĚJI ŽÁDNÉ JMÉNO NEŽ ŠPATNÉ. Vymyšlené jméno je horší než prázdné pole, protože působí důvěryhodně a projde dál.',
            '- IČO NEHÁDEJ. Vrať ho POUZE pokud jsi ho opravdu viděl v konkrétním zdroji (justice.cz / ARES detail firmy). Když si nejsi jistý, vrať "ico": null — aplikace si IČO sama spolehlivě dohledá v ARES podle NÁZVU firmy. Vymyšlené 8místné IČO je HORŠÍ než null, protože přiřadí kontakt ke špatné firmě a vypadá důvěryhodně.',
            '- Nejdůležitější je proto PŘESNÝ NÁZEV FIRMY (podle něj se dohledá IČO i sídlo). Raději ověřený název bez IČO než smyšlené IČO.',
            '- Do "zdrojUrl" vždy uveď konkrétní URL, kde jsi osobu a firmu ověřil. Bez ověřitelného zdroje osobu neuváděj.',
            '',
            ...
            '- Pokud nemůžeš ověřit konkrétní profilovou URL, vrať **Google site:linkedin.com/in search URL** v tomto formátu:',
            '  https://www.google.com/search?q=site%3Alinkedin.com%2Fin+Jméno+Příjmení+Značka',
            '  (URL-encoded `site:linkedin.com/in Jméno Příjmení ZnačkaFirmy`. PRAVIDLA: (1) Jméno ANI firmu NEDÁVEJ do uvozovek — jen volná klíčová slova; uvozovky kolem celého jména v EU naráží na GDPR filtr Googlu (osobní profily se z přesné shody odstraní) → 0 výsledků. (2) Z firmy použij JEN jedno značkové slovo (napiš "ATALIAN", ne "ATALIAN CZ s.r.o."; "ProfiLend", ne "ProfiLend Group s.r.o.") — Google site: dělá AND přes všechna slova, takže každé slovo navíc, které není doslova na profilu, srazí výsledek klidně na nulu. Vynech právní formu, zemi (CZ/SK) i obecná slova (Skupina, Group, Holding). Google relevanci seřadí sám — správná osoba bývá první. NEPOUŽÍVEJ LinkedIn vlastní /search/results/people/ URL — bez přihlášení vrací prázdno a dělá AND-match přes klíčová slova.)',
            '- Pokud nemáš ani jméno (jen firmu), vrať Google search pro company stránku: https://www.google.com/search?q=site%3Alinkedin.com%2Fcompany+Firma',
            (ctx.preferLinkedIn
                ? '\n- PRIORITA LINKEDIN: Pro tuto variantu vraceť POUZE kontakty, kde umíš dodat OVĚŘENOU profilovou URL (/in/{slug}). Google search URL je akceptovatelná jako fallback. Kontakty, kde nemáš ani jméno (chybí keywords pro Google search), zcela vyřaď.'
                : ''),
            thoroughInstr,
            '',
            'SEBE-KONTROLA PŘED ODESLÁNÍM (POVINNÉ — projdi si celý výstup ještě jednou):',
            '- POZOR, CO A JAK VYHLEDÁVÁŠ: hledej cíleně a ověřuj. Pro každou osobu si dohledej potvrzení v obchodním rejstříku (justice.cz / ARES podle IČO nebo názvu) nebo na oficiálním webu firmy. Nespoléhej na odhad ani na paměť — bez konkrétního výsledku z web search nech pole null.',
            '- POZOR NA ZÁMĚNY: ověř, že jméno + firma + IČO patří k SOBĚ (stejný subjekt). Ne každý jmenovec u podobně znějící firmy je ten správný — pozor na pobočky, přejmenované firmy a shodu jmen.',
            '- REVIZE KAŽDÉHO KONTAKTU: u každého si polož otázku „Mám pro toto jméno a roli konkrétní zdroj, který bych dokázal ukázat?" Pokud ne → kontaktniOsoba i role = null.',
            '- Radši vrať MÉNĚ kontaktů, které jsou 100% ověřené, než víc s odhadnutými jmény. Jedno vymyšlené jméno poškodí důvěru v celý výstup.',
            '',
            ...
            '  "contacts": [',
            '    {',
            '      "firma": "PŘESNÝ název firmy vč. právní formy (povinné — podle něj se dohledá IČO)",',
            '      "ico": "8místné IČO JEN pokud ověřené z rejstříku, jinak null (nehádej!)",',
            '      "kontaktniOsoba": "Jméno Příjmení | null",',
            '      "role": "Managing Partner | null",',
            '      "email": "veřejný email | null",',
            '      "telefon": "veřejný tel | null",',
            '      "web": "https://… | null",',
            '      "linkedinUrl": "BUĎ ověřená profilová URL https://linkedin.com/in/{slug} NEBO Google search URL https://www.google.com/search?q=site%3Alinkedin.com%2Fin+Jméno+Příjmení+Značka (jméno + JEDNO značkové slovo firmy, vše BEZ uvozovek!) NEBO null",',
            '      "sidlo": "adresa | null",',
    ...
    if (mode === 'deep') {
        ...
        return [
            ...
            '- LinkedIn URL: pokud profil OVĚŘÍŠ přes web search, vrať /in/{slug}. Pokud ne, vrať Google search URL https://www.google.com/search?q=site%3Alinkedin.com%2Fin+Jméno+Příjmení+Značka (jméno + JEDNO značkové slovo firmy, vše BEZ uvozovek — uvozovky kolem jména v EU naráží na GDPR filtr Googlu → 0 výsledků). Nikdy halucinovat /in/jmeno-prijmeni — LinkedIn slugy obsahují hash.',
            thoroughInstr,
            '',
            (ctx.manual ? 'STRUKTURA JSON ODPOVĚDI (jak ji obalit viz pokyn na konci):' : 'FORMÁT ODPOVĚDI: vrať POUZE platný JSON, bez markdown obalu.'),
            '{',
