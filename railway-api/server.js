...
function buildAIDiscoveryPrompt(mode, ctx) {
    ...
    if (mode === 'contacts') {
        ...
        return [
            ...
            '  https://www.google.com/search?q=site%3Alinkedin.com%2Fin+%22Jméno+Příjmení%22+%22Firma%22',
            '  (URL-encoded `site:linkedin.com/in "Jméno Příjmení" "Firma"`. Google index obsahuje všechny veřejné LinkedIn profily a první výsledek je téměř vždy správný profil — uživatel klikne a otevře se mu reálná /in/ URL. NEPOUŽÍVEJ LinkedIn vlastní /search/results/people/ URL — bez přihlášení vrací prázdno a dělá AND-match přes klíčová slova.)',
            '- Pokud nemáš ani jméno (jen firmu), vrať Google search pro company stránku: https://www.google.com/search?q=site%3Alinkedin.com%2Fcompany+%22Firma%22',
            (ctx.preferLinkedIn
            ...
            '      "web": "https://… | null",',
            '      "linkedinUrl": "BUĎ ověřená profilová URL https://linkedin.com/in/{slug} NEBO Google search URL https://www.google.com/search?q=site%3Alinkedin.com%2Fin+\\"Jméno\\"+\\"Firma\\" NEBO null",',
            '      "sidlo": "adresa | null",',
    ...
    if (mode === 'deep') {
        ...
        return [
            ...
            '- LinkedIn URL: pokud profil OVĚŘÍŠ přes web search, vrať /in/{slug}. Pokud ne, vrať Google search URL https://www.google.com/search?q=site%3Alinkedin.com%2Fin+"Jméno"+"Firma". Nikdy halucinovat /in/jmeno-prijmeni — LinkedIn slugy obsahují hash.',
            thoroughInstr,
            '',
            (ctx.manual ? 'STRUKTURA JSON ODPOVĚDI (jak ji obalit viz pokyn na konci):' : 'FORMÁT ODPOVĚDI: vrať POUZE platný JSON, bez markdown obalu.'),
            '{',
