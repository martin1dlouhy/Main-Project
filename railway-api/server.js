...
function buildAIDiscoveryPrompt(mode, ctx) {
    ...
    if (mode === 'contacts') {
        ...
        return [
            ...
            '- LIMIT NA FIRMU: u firem, které už mám v seznamu existujících kontaktů 3× a víc, NEhledej další osoby — radši najdi NOVÉ firmy (v DB platí max 3 kontakty na firmu).',
            ...
            '- Pokud nemůžeš ověřit konkrétní profilovou URL, vrať **obyčejný Google vyhledávací dotaz** v tomto formátu:',
            '  https://www.google.com/search?q=Jméno+Příjmení+Značka+LinkedIn',
            '  (URL-encoded `Jméno Příjmení ZnačkaFirmy LinkedIn`. PRAVIDLA: (1) NEPOUŽÍVEJ operátor site:linkedin.com/in — v EU kvůli GDPR delistingu osobních profilů vrací skoro nulu / náhodný šum. Obyčejný dotaz funguje líp, Google správný profil seřadí první. (2) BEZ uvozovek — přesná fráze jména naráží na GDPR filtr → 0 výsledků. (3) Z firmy JEN jedno značkové slovo (napiš "Dentons", ne "Dentons Europe LLP"; "ATALIAN", ne "ATALIAN CZ s.r.o.") — vynech právní formu, zemi i obecná slova. (4) Přidej na konec slovo "LinkedIn" — nasměruje Google na profil.)',
            '- Pokud nemáš jméno osoby (jen firmu), vrať "linkedinUrl": null — hledáme profil konkrétního člověka, ne firemní stránku.',
            (ctx.preferLinkedIn
            ...
            '      "web": "https://… | null",',
            '      "linkedinUrl": "BUĎ ověřená profilová URL https://linkedin.com/in/{slug} NEBO obyčejný Google dotaz https://www.google.com/search?q=Jméno+Příjmení+Značka+LinkedIn (BEZ site: a BEZ uvozovek; jméno + jedno značkové slovo + slovo LinkedIn) NEBO null (když nemáš jméno osoby)",',
            '      "sidlo": "adresa | null",',
    ...
    if (mode === 'deep') {
        ...
        return [
            ...
            '- LinkedIn URL: pokud profil OVĚŘÍŠ přes web search, vrať /in/{slug}. Pokud ne, vrať obyčejný Google dotaz https://www.google.com/search?q=Jméno+Příjmení+Značka+LinkedIn (jméno + JEDNO značkové slovo firmy + slovo LinkedIn, BEZ uvozovek a BEZ operátoru site: — site:linkedin.com/in v EU vrací kvůli GDPR skoro nulu). Bez jména osoby vrať null. Nikdy halucinovat /in/jmeno-prijmeni — LinkedIn slugy obsahují hash.',
            thoroughInstr,
            '',
            (ctx.manual ? 'STRUKTURA JSON ODPOVĚDI (jak ji obalit viz pokyn na konci):' : 'FORMÁT ODPOVĚDI: vrať POUZE platný JSON, bez markdown obalu.'),
            '{',
